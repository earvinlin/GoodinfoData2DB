...
執行程式語法：
<windows>
python Chrome_GetGoodinfoBzPerformanceData2sre_2_nobrowser.py STOCKS_LIST_test.txt test 0
<imac / linux>
...
#  Process one stock
# ------------------------------------------------------------
def process_stock_once(driver, stockCode, destination_dir, theDirectory, theSelectOption, download_dir) :
...
#  Retry wrapper
# ------------------------------------------------------------
def process_stock_with_retry(stockCode, destination_dir, theDirectory, theSelectOption, download_dir, logFile) :
    stock_filename = f"{stockCode}-bzPerformance-{theDirectory}.xls"
    ...
    for attempt in range(1, maxRetryCnt + 1):
        ...
        try:
            ...
            if process_stock_once(driver, stockCode, destination_dir, theDirectory, theSelectOption, download_dir):
                return
...
#    print("下載暫存資料夾：", download_dir)
    ...
    with open(logFilename, "a", encoding="utf-8") as logFile, open(theStocksList, "r", encoding="utf-8") as f:
        for processCnt, line in enumerate(f, start=1):
            ...
            process_stock_with_retry(stockCode, destination_dir, theDirectory, theSelectOption, download_dir, logFile)
